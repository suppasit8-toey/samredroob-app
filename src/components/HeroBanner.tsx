...
const slides = [
    ...
    },
    {
        ...
        link: "/products?category=Wallpaper",
...
export default function HeroBanner() {
    ...
    return (
        <div className="relative w-full h-[400px] md:h-[500px] rounded-2xl overflow-hidden shadow-2xl mb-8 group bg-gray-900">
            ...
            <div className="absolute inset-0 flex flex-col justify-center px-6 md:px-16 text-white z-20 max-w-4xl">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-sm font-medium mb-4 text-white border border-white/20">
                        ...
                        <p className="text-lg md:text-xl text-white/90 mb-8 font-light drop-shadow-sm max-w-lg">
                            {slide.subtitle}
                        </p>
                        <Link href={slide.link}>
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                className="bg-white text-black px-8 py-3 rounded-full font-bold text-lg flex items-center gap-2 shadow-lg hover:shadow-xl transition-all"
                            >
            ...
            {/* Navigation Arrows */}
            <button
                className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity z-30"
                onClick={() => setCurrentSlide(prev => (prev - 1 + slides.length) % slides.length)}
            >
                <ArrowRight className="transform rotate-180" size={24} />
            </button>
            <button
                className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity z-30"
